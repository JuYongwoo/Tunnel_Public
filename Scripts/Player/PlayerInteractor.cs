    ...
    bool HandleInteraction(ItemType type, GameObject obj, string message)
    {
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            else if (type == ItemType.Door)
            {
                ...
                if (ManagerObject.instance.actionManager.OnFindInvenAction(obj.GetComponent<Door>().KeyName)) // 해당 문의 door class의 KeyName이 인벤토리에 있는지 확인
                {
                    obj.transform.Rotate(0, 90, 0); //Y 90도 회전
                    obj.GetComponent<Collider>().enabled = false;
                    ManagerObject.instance.sound.PlayAudioClip(ManagerObject.instance.resource.soundsmap[SoundsEnum.doorsound].Result, 0.3f, false);
                }
                else
                {
